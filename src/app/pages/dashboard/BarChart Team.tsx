...
const BarChartTeam: React.FC<Props> = ({
  className,
  chartColor,
  chartHeight,
  dashboardData,
    ...
    <div className={`card ${className}`}>
      <div className="card-body p-0 d-flex justify-content-between flex-column overflow-hidden">
        <div className="d-flex flex-stack flex-wrap flex-grow-1 px-9 pt-9 pb-3">
          <div className="me-2">
            <span className="fw-bold text-gray-800 d-block fs-3">Team</span>
          </div>
        </div>
...
const chartOptions = (
  ...
  const labelColor = getCSSVariableValue("--bs-white-500");
  ...
  return {
    ...
    dataLabels: {
      ...
      offsetY: -20, // Position data labels above the bars
      style: {
        fontSize: "12px",
        colors: [labelColor],
      },
    ...
    xaxis: {
      categories:
        chartData?.length > 0
          ? chartData?.map((item: any) => item?.team_name)
          : [],
      axisBorder: {
        show: false,
      },
